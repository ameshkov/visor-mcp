...
describe('loadImage data URL', () => {
  it('accepts a valid base64 PNG data URL and canonicalizes it', async () => {
    const image = await loadImage(TINY_PNG_DATA_URL, 5);
  ...
  it('rejects malformed base64', async () => {
    await expect(loadImage('data:image/png;base64,!!!notbase64!!!', 5)).rejects.toThrow(/base64/);
  });

  it('rejects a non-base64 data URL', async () => {
    await expect(loadImage('data:image/png,abc', 5)).rejects.toThrow(/base64 data URL/);
  });

  it('rejects a non-data-url plain string', async () => {
    await expect(loadImage('not a data url', 5)).rejects.toThrow(/base64 data URL/);
  });
...
describe('loadImage local file', () => {
  ...
  it('rejects a relative path', async () => {
    await expect(loadImage('relative/path.png', 5)).rejects.toThrow(/absolute file path/);
  });
  ...
  it('rejects a local file exceeding the configured size limit', async () => {
    ...
    const file = writeTempFile(TINY_PNG_BYTES);
    try {
      await expect(loadImage(file.path, 1e-5)).rejects.toThrow(/size limit/);
    } finally {
      file.cleanup();
    }
  });
...
describe('loadImage local file rejection', () => {
  it('rejects a missing file as not found', async () => {
    ...
    await expect(loadImage(missing, 5)).rejects.toThrow(/was not found/);
  });
  ...
  it.skipIf(!onPosix || isRoot)('rejects an unreadable file', async () => {
    ...
    try {
      ...
      await expect(loadImage(file.path, 5)).rejects.toThrow(/could not be read/);
    } finally {
  ...
  it('rejects a directory', async () => {
    ...
    try {
      await expect(loadImage(dir.path, 5)).rejects.toThrow(/is not a file/);
    } finally {
      dir.cleanup();
    }
  });
  ...
  it.skipIf(!existsSync('/dev/null'))('rejects a non-file device', async () => {
    await expect(loadImage('/dev/null', 5)).rejects.toThrow(/is not a file/);
  });
});
...
describe('loadImage scheme classification', () => {
  it.each([
    ...
    await expect(loadImage(source, 5)).rejects.toThrow(/scheme is not supported/);
  });
});
...
describe('loadImage http url', () => {
  ...
  it('does not transmit the URL fragment', async () => {
    await loadImage(`${server.url}/png#section`, 5);
  ...
  it('does not transmit URL credentials', async () => {
    ...
    await loadImage(`${withCreds}/png`, 5);
    expect(server.requests[0].authorization).toBeUndefined();
    expect(server.requests[0].cookie).toBeUndefined();
  });

  it('transmits the query string but redacts it from errors', async () => {
    const err = await loadImage(`${server.url}/missing?secret=value`, 5).catch((e) => e);
  ...
  it('rejects a non-2xx response as a download failure', async () => {
    ...
    await expect(loadImage(`${server.url}/gone`, 5)).rejects.toThrow(/download failed/);
  });

  it('rejects a non-image 200 response', async () => {
    server.setRoute('/html', { status: 200, body: '<html></html>' });
    await expect(loadImage(`${server.url}/html`, 5)).rejects.toThrow(/supported format/);
  });

  it('rejects an oversized response', async () => {
    server.setRoute('/big', { status: 200, body: Buffer.alloc(10, 0x89) });
    await expect(loadImage(`${server.url}/big`, 1e-6)).rejects.toThrow(/size limit/);
  });

  it('accepts an HTTPS URL (scheme validated, fetch attempted)', async () => {
    // HTTPS to a plain-HTTP mock server fails the TLS handshake; the error is a
    // download failure, NOT a scheme rejection, proving https is accepted.
    const httpsUrl = server.url.replace('http://', 'https://');
    const err = await loadImage(`${httpsUrl}/png`, 5).catch((e) => e);
    expect(err).toBeInstanceOf(Error);
    expect((err as Error).message).toMatch(/download failed/);
...
describe('loadImage http redirects', () => {
  ...
  it('follows a redirect chain to the final image', async () => {
    ...
    const image = await loadImage(`${server.url}/start`, 5);
    expect(image.mimeType).toBe('image/png');
    expect(image.dataUrl).toBe(`data:image/png;base64,${TINY_PNG_BASE64}`);
  });
  ...
  it('follows exactly five redirects', async () => {
    ...
    const image = await loadImage(`${server.url}/r0`, 5);
    expect(image.mimeType).toBe('image/png');
  });
  ...
  it('rejects when redirects exceed five', async () => {
    ...
    await expect(loadImage(`${server.url}/r0`, 5)).rejects.toThrow(/redirect limit/);
  });
  ...
  it('rejects a redirect to an unsupported scheme', async () => {
    ...
    await expect(loadImage(`${server.url}/start`, 5)).rejects.toThrow(/scheme is not supported/);
  });

  it('rejects a redirect with no location header', async () => {
    server.setRoute('/start', { status: 302 });
    await expect(loadImage(`${server.url}/start`, 5)).rejects.toThrow(/location/);
  });
  ...
  it('accepts an HTTPS redirect target', async () => {
    ...
    const err = await loadImage(`${server.url}/start`, 5).catch((e) => e);
    expect(err).toBeInstanceOf(Error);
    expect((err as Error).message).toMatch(/download failed/);
    expect((err as Error).message).not.toMatch(/scheme is not supported/);
  });
