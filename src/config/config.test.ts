...
describe('loadConfig precedence', () => {
  it('process env wins over .env and .env fills unset values', () => {
    const dir = tmpDirWithEnv(
      'VISION_MCP_API_KEY=fromfile\nVISION_MCP_BASE_URL=https://file.test/v1\nVISION_MCP_MODEL=filemodel\n',
    );
    try {
      const cfg = loadConfig({ env: { VISION_MCP_API_KEY: 'fromprocess' }, cwd: dir });
...
describe('loadConfig required-field validation', () => {
  it.each([
    [
      'missing api key',
      { VISION_MCP_BASE_URL: 'https://x.test', VISION_MCP_MODEL: 'm' },
      /API_KEY/,
    ],
    ['missing base url', { VISION_MCP_API_KEY: 'k', VISION_MCP_MODEL: 'm' }, /BASE_URL/],
    ['missing model', { VISION_MCP_API_KEY: 'k', VISION_MCP_BASE_URL: 'https://x.test' }, /MODEL/],
    ['empty api key', { ...valid, VISION_MCP_API_KEY: '   ' }, /API_KEY/],
...
describe('loadConfig base URL scheme', () => {
  it.each([
    ...
    expect(() => loadConfig(withEnv({ ...valid, VISION_MCP_BASE_URL: baseUrl }))).toThrow(pattern);
  });

  it('accepts http and https', () => {
    const http = loadConfig(withEnv({ ...valid, VISION_MCP_BASE_URL: 'http://localhost:8080' }));
    const https = loadConfig(withEnv({ ...valid, VISION_MCP_BASE_URL: 'https://x.test' }));
...
describe('endpoint composition', () => {
  it.each([
    ...
    const cfg = loadConfig(withEnv({ ...valid, VISION_MCP_BASE_URL: baseUrl }));
...
describe('request extras validation', () => {
  it('parses a valid object', () => {
    const cfg = loadConfig(
      withEnv({ ...valid, VISION_MCP_REQUEST_BODY_JSON: '{"reasoning_effort":"high"}' }),
    );
  ...
  it.each([
    ...
    expect(() => loadConfig(withEnv({ ...valid, VISION_MCP_REQUEST_BODY_JSON: raw }))).toThrow(
      pattern,
    );
  });
});
...
describe('config immutability', () => {
  it('returns a frozen config with deep-frozen extras', () => {
    const cfg = loadConfig(withEnv({ ...valid, VISION_MCP_REQUEST_BODY_JSON: '{"a":{"b":1}}' }));
